...
export function getArtClassSpec(vm) {
  ...
  vm.perform(env => {
    ...
    try {
      ...
      // Android 16 (API 36) changed: copied_methods_offset_ stores an INDEX into methods array,
      // not the total count. We need different detection logic.
      const apiLevel = getAndroidApiLevel();

      if (apiLevel >= 36) {
        // copied_methods_offset_ and virtual_methods_offset_ are indices into methods array.
        // They can be 0 if there are no copied/virtual methods.
        // The known offset for these fields in Android 16 is 0x64 (copied) and 0x66 (virtual).

        // Try the known offset first
        const knownOffset = 0x64;
        const copiedIdx = object.add(knownOffset).readU16();
        const virtualIdx = object.add(knownOffset + 2).readU16();

        // Validate: both should be <= methodsArraySize (can be 0 or equal to size)
        if (copiedIdx <= methodsArraySize && virtualIdx <= methodsArraySize) {
          offsetCopiedMethods = knownOffset;
        }

        // If known offset doesn't work, try scanning
        if (offsetCopiedMethods === -1) {
          for (let offset = offsetMethods + 4; offset < MAX_OFFSET - 2; offset += 4) {
            const val1 = object.add(offset).readU16();      // copied_methods_offset_
            const val2 = object.add(offset + 2).readU16();  // virtual_methods_offset_

            // Valid if both are <= methodsArraySize
            if (val1 <= methodsArraySize && val2 <= methodsArraySize) {
        ...
        // Original logic for Android 15 and below: find u16 == methodsArraySize
